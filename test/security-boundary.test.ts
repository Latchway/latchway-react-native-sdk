...
describe("JavaScript security boundary", () => {
  ...
  it("uses opaque native dispatch and never returns protocol credentials through the bridge", async () => {
    ...
    expect(spec).not.toContain("revokeFamilyWithComponents(");
    ...
    expect(ios).toContain("runtime: .reactNativeIOS");
    expect(ios).not.toContain("legacySharedKeychainAccessGroups");
    expect(android).not.toContain("TransientIdentityTokenProvider");
    expect(ios).toContain("runComponent(clientID:");
    expect(android).toContain("ATTESTATION_UNSUPPORTED");
    ...
    expect(ios).not.toContain("revokeCurrentInstallationFamily(retiring:");
  ...
  it("isolates the extension-process client from the containing root lease and identity callback", async () => {
    ...
    expect(componentContext).not.toContain("rootKeychainAccessGroup:");
    expect(componentContext).toContain("account: account");
    expect(componentContext).toContain("keychainAccessGroup: input.keychainAccessGroup");
    expect(types).not.toContain("establishDirectAttestation");
    expect(types).toContain("account: string");
  });
  ...
  it("FW-AUTH-104 and FW-AUTH-105 keep iOS identity and family work inside the native client", async () => {
    ...
    expect(ios).not.toContain("TransientIdentityTokenProvider");
    expect(ios).not.toContain("identityToken:");
