...
test('installs stable async symbols before stream classes evaluate', () => {
  const source = fs.readFileSync(path.join(__dirname, '../src/runtime/symbols.ts'), 'utf8');
  const compiled = transformSync(source, {
    filename: 'symbols.ts', configFile: require.resolve('../babel.config.js'),
  }).code;
...
test('the entrypoint initializes symbols before stream dependencies', () => {
  ...
  const bootstrap = entry.indexOf("import './src/runtime/polyfills'");
